...
class SchedulerProvider {
  ...
  /**
   * Starts a CRON-based scheduled task.
   * @param {*} task The task/activity to schedule (usually JSON object).
   * @param {string} cron The CRON expression defining the schedule.
   * @param {string=} taskName Optional name for the task (defaults to task type or ID).
   * @return {Promise<void>} A promise that resolves when the task is started.
   */
  async startCron(task, cron, taskName) {
    // Generate a task name if not provided
    const name = taskName || (task && task.type) || `task-${Date.now()}`;

    // Store task metadata for retrieval
    const metadata = {
      task,
      cron,
    ...
    // Track schedule as started with CRON and activity info
   ...
   * @return {Promise<void>} A promise that resolves when the task is canceled.
   */
  async cancel(taskId) {
    if (this.tasks_.has(taskId)) {
      const task = this.tasks_.get(taskId);

      // Clear interval if it's an interval-based task
      if (task.intervalId) {
