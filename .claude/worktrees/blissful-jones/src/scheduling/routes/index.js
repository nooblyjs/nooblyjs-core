...
module.exports = (options, eventEmitter, scheduler) => {
  if (options['express-app'] && scheduler) {
    ...
    app.post('/services/scheduling/api/schedule', async (req, res) => {
      ...
      if (task && cron) {
        try {
          await scheduler.startCron(task, cron);
          res.status(200).json({ status: 'OK', message: 'Schedule created successfully' });
        } catch (err) {
          res.status(500).json({ error: err.message });
        }
      } else {
        res.status(400).json({ error: 'Bad Request: Missing task or cron expression' });
      }
    });
     ...
     * Returns list of all schedules with details for UI.
     *
     * @param {express.Request} req - Express request object
     * @param {express.Response} res - Express response object
     * @return {void}
     */
    app.get('/services/scheduling/api/schedules', (req, res) => {
      try {
     ...
     *
     * @param {express.Request} req - Express request object
     * @param {string} req.params.scheduleId - The ID of the schedule
     * @param {express.Response} res - Express response object
     * @return {void}
     */
    app.get('/services/scheduling/api/executions/:scheduleId', (req, res) => {
      try {
        const scheduleId = req.params.scheduleId;
        const limit = parseInt(req.query.limit) || 20;
        const executions = analytics.getExecutionHistory(scheduleId, limit);
