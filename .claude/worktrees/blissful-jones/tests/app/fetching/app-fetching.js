...
// Load images
app.use('/images/s-tech-logo-colour.png', express.static(path.join(__dirname, 's-tech-logo-colour.png')));
...
app.listen(process.env.PORT || 3101, async () => {
  ...
  const runFetchTest = async () => {
    ...
    const endpoints = [
      'http://localhost:3101/services/fetching/api/status/?number=1',
      'http://localhost:3101/services/fetching/api/status/?number=2',
      'http://localhost:3101/services/fetching/api/status/?number=3',
      'http://localhost:3101/services/fetching/api/status/?number=4',
      'http://localhost:3101/services/fetching/api/status/?number=5',
      'http://localhost:3101/services/fetching/api/status/?number=6',
      'http://localhost:3101/services/fetching/api/status/?number=7',
      'http://localhost:3101/services/fetching/api/status/?number=8',
      'http://localhost:3101/services/fetching/api/status/?number=9',
      'http://localhost:3101/services/fetching/api/status/?number=10'
    ];
  ...
  setInterval(runFetchTest, 1000);

  // Run immediately on startup for testing
  await runFetchTest();

  await logger.info('Scheduled fetch tests to run every 60 seconds');
  await logger.info('Each batch will make 1000 requests with caching enabled');
