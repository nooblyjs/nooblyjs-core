...
describe('DataService', () => {
  ...
  describe('InMemoryDataServiceProvider', () => {
    ...
    it('should auto-create container when adding to non-existent container', async () => {
      const user = { name: 'Alice' };
      const uuid = await dataService.add('nonExistentContainer', user);
      expect(uuid).toBeDefined();
      expect(typeof uuid).toBe('string');
      expect(dataService.provider.containers.has('nonExistentContainer')).toBe(true);
    });
  });
