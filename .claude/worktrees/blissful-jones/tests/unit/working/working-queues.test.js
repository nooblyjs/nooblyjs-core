...
describe('Working Service with Named Queues', () => {
  ...
  it('should add tasks to incoming queue', async () => {
    ...
    const incomingQueueSize = await queueService.size('digital-technologies-core-working-incoming');
  ...
  it('should have correct queue names defined', () => {
    const incomingQueueName = 'digital-technologies-core-working-incoming';
    const completeQueueName = 'digital-technologies-core-working-complete';
    const errorQueueName = 'digital-technologies-core-working-error';

    // Verify queue names are accessible through queue service
    expect(incomingQueueName).toBe('digital-technologies-core-working-incoming');
    expect(completeQueueName).toBe('digital-technologies-core-working-complete');
    expect(errorQueueName).toBe('digital-technologies-core-working-error');
  });

  it('should maintain separate queue sizes', async () => {
    // Get initial sizes
    const initialIncoming = await queueService.size('digital-technologies-core-working-incoming');
    const initialComplete = await queueService.size('digital-technologies-core-working-complete');
    const initialError = await queueService.size('digital-technologies-core-working-error');
