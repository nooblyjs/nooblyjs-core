...
|---------|---------|-----------|---------------|
| **aiservice** | LLM integration for AI prompts with token tracking | claude, chatgpt, ollama | `/services/ai/api/*` |
...
AI Service provides LLM integration for Claude, ChatGPT, and Ollama with token usage tracking and cost estimation.

**Configuration Requirements:**
- **Claude**: Requires `apiKey` in constructor options
- **ChatGPT**: Requires `apiKey` in constructor options
- **Ollama**: Requires local Ollama server at `baseUrl` (default: `http://localhost:11434`)
- Token usage data is automatically stored in local JSON files (configurable via `tokensStorePath`)

```javascript
// Initialize AI services with connection info in constructor
const claudeAI = serviceRegistry.aiservice('claude', {
  apiKey: process.env.ANTHROPIC_API_KEY,
  model: 'claude-3-5-sonnet-20241022', // Optional
  tokensStorePath: './.data/ai-tokens-claude.json' // Optional
});
...
// Send prompts
const response = await claudeAI.prompt('Explain quantum computing', {
  maxTokens: 1000,
  temperature: 0.7
});
...
# Response: "aiservice api running"
```

**Key Features:**
- Automatic token tracking and cost estimation
- Provider-agnostic interface (Claude/ChatGPT/Ollama)
- Local JSON storage for usage data (no DataServe required)
- Event emission for AI operations (`ai:usage`, `ai:prompt`, `ai:error`)
- Analytics for model usage and cost tracking
...
```javascript
// Initialize AI services - connection info MUST be provided in constructor
const claudeAI = serviceRegistry.aiservice('claude', {
  apiKey: process.env.ANTHROPIC_API_KEY,
  model: 'claude-3-5-sonnet-20241022',
...
const chatGPT = serviceRegistry.aiservice('chatgpt', {
  apiKey: process.env.OPENAI_API_KEY,
  model: 'gpt-3.5-turbo',
  tokensStorePath: './.data/ai-tokens-chatgpt.json'
});

const ollama = serviceRegistry.aiservice('ollama', {
  baseUrl: 'http://localhost:11434',
  model: 'llama3.2',
  tokensStorePath: './.data/ai-tokens-ollama.json'
});

// Send prompts and get responses
const response = await claudeAI.prompt('Explain quantum computing in simple terms', {
  maxTokens: 1000,
  temperature: 0.7
});
...
console.log('Estimated Cost:', response.usage.estimatedCost);

// Get analytics for cost tracking
const analytics = claudeAI.getAnalytics();
console.log('Total Cost:', analytics.totalCost);
console.log('Total Tokens:', analytics.totalTokens);
console.log('Model Usage:', analytics.modelUsage);
...
// Real-world example: Batch processing with Ollama (free local LLM)
async function summarizeDocuments(documents) {
  const summaries = [];

  for (const doc of documents) {
    const response = await ollama.prompt(`Summarize this document:\n\n${doc.content}`, {
      maxTokens: 500,
      temperature: 0.5
    });
