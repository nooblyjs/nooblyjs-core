...
module.exports = (options, eventEmitter, auth, analytics) => {
  if (options['express-app'] && auth) {
    ...
    const healthCheck = new HealthCheck('authservice', { dependencies: [] });
     ...
     */
    app.get('/services/authservice/api/health', authMiddleware || ((req, res, next) => next()), async (req, res) => {
      try {
        const result = await healthCheck.check({ service: auth });
    ...
    /**
     * POST /services/authservice/api/import
     ...
     * @return {void}
     */
    app.post('/services/authservice/api/import', authMiddleware || ((req, res, next) => next()), async (req, res) => {
      try {
        ...
        handleError(res, error, { operation: 'authservice-import' });
      }
    });
