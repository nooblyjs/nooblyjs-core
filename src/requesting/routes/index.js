...
module.exports = (options, eventEmitter, queue) => {
  if (options['express-app'] && queue) {
     ...
     * @return {void}
     */
    app.post('/services/queueing/api/enqueue/:queueName', async (req, res) => {
     ...
     * @return {void}
     */
    app.get('/services/queueing/api/dequeue/:queueName', async (req, res) => {
      const {queueName} = req.params;
      if (!queueName) {
        return res.status(400).send('Bad Request: Missing queue name');
      }
     ...
     * @return {void}
     */
    app.get('/services/queueing/api/size/:queueName', async (req, res) => {
      const {queueName} = req.params;
      if (!queueName) {
        return res.status(400).send('Bad Request: Missing queue name');
      }
     ...
     * @return {void}
     */
    app.get('/services/queueing/api/queues', async (req, res) => {
      try {
     ...
     * @return {void}
     */
    app.delete('/services/queueing/api/purge/:queueName', async (req, res) => {
      const {queueName} = req.params;
      if (!queueName) {
        return res.status(400).send('Bad Request: Missing queue name');
      }
