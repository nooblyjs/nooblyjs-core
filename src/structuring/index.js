...
module.exports = {
     ...
     * 
     * @param {object} name -  The name of the application
     ...
     */
    mount: function (name, app, serviceRegistry, eventEmitter, baseUrl, options) {

        // Determine where we are working
        console.log(`nooblysjs-core-structure: ${name} current working: ${process.cwd()}`);
        ...
        if (fs.existsSync('./src/services/')) {
            this.mountFiles('./src/services/',name,  app, serviceRegistry, eventEmitter, baseUrl, options)
        ...
        if (fs.existsSync('./src/data/index.js')) {
            this.mountFiles('./src/data/',name,  app, serviceRegistry, eventEmitter, baseUrl, options)
        ...
        if (fs.existsSync('./src/routes/index.js')) {
            this.mountFiles('./src/routes/',name,  app, serviceRegistry, eventEmitter, baseUrl, options)
        ...
        if (fs.existsSync('./src/views/index.js')) {
            this.mountFiles('./src/views/',name,  app, serviceRegistry, eventEmitter, baseUrl, options)
            ...
            console.log('nooblysjs-core-structure: views static data loaded')
        }

        // Determine if activites folder and index file exists and load it
        if (fs.existsSync('./src/activities/index.js')) {
            this.mountFiles('./src/activities/',name,  app, serviceRegistry, eventEmitter, baseUrl, options)
        ...
        if (fs.existsSync('./public/styles.css')) {
            ...
            console.log('nooblysjs-core-structure: base styles loaded')
        }
    },
     ...
     * @param {} path 
     */
    mountFiles: function(path, name,  app, serviceRegistry, eventEmitter, baseUrl, options){
    const files = fs.readdirSync('./src/services/');
        files.forEach(file => {
            if (path.extname(file) === '.js') {
                const MountedFile = require(file);
                MountedFile(name, app, serviceRegistry, eventEmitter,baseUrl ,options);
            }
        });
    }
