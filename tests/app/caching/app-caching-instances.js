...
const cacheFile = serviceRegistry.getService('caching', 'file', { instanceName: 'file' });
const cacheMemcached = serviceRegistry.getService('caching', 'memcached', { instanceName: 'memcached' });
const cacheRedis = serviceRegistry.getService('caching', 'redis', { instanceName: 'redist' });
  ...
  try {
    ...
    for (let i = 0; i < 1000; i++) {
      ...
      try {
        ...
        await cacheFile.put(key, value, 600); // 10 minute TTL
        await cacheMemcached.put(key, value, 600); // 10 minute TTL
        await cacheRedis.put(key, value, 600); // 10 minute TTL
    ...
    for (let i = 0; i < 100; i++) {
      ...
      try {
        ...
        const valueFile = await cacheFile.get(key);
        const valueMemcached = await cacheMemcached.get(key);
        const valueRedis = await cacheRedis.get(key);        
       
        if (value && valueMemcached && valueRedis) {
    ...
    for (let i = 0; i < 100; i++) {
      ...
      try {
        ...
        await cacheFile.delete(key); 
        await cacheMemcached.delete(key); 
